...
create.ps(sol.file=sol.file, ps.name=ps.name, user.name=NULL,libs=libs, stop.when.finished=FALSE, addons="quiz", var.txt.file = "gasoline_taxes_var.txt")
...
The information of the `summary()` command is divided into for parts: *Call*, *Residuals*, *Coefficients* and *Statistics*. *Call* shows the formula that R used to fit the data - the given `lm()` command. The *Residuals* section displays some summary statistics for the residuals of the fitted model, such as the median.
...
+ *Std. Error* is the **standard error of the coefficient**. Standard errors measure the average amount that the estimates differ from the actual average value of the explanatory variable. Thus, smaller standard errors relative to the estimators are preferred.
...
$$\tilde{RetailPrice_{st}}=\tilde{\beta_{0}} + \tilde{\beta_{1}}\tau_{st} +\epsilon_{st}$$

The "~" emphasizes that $\tilde{\beta_{1}}$ comes from an underspecified model. 

We want to derive the bias in the simple regression estimator of $\tilde{\beta_{1}}$.
...
Note, that $\tilde{\beta}_0$ and $\tilde{\beta}_1$ are the estimators of the short regression$(1)$ and $\hat{\beta}_0$, $\hat{\beta}_1$ and $\hat{\beta}_2$ denote the estimators of the long regression $(2)$.

$\tilde{\delta_{1}}$ is the slope parameter $\delta_{1}$ of the simple regression of `nom_oilprice` on `gas_tax_all`: 

$$OilPrice_{t}=\alpha+\tilde{\delta_{1}}\tau_{st}+\epsilon_{st}.$$
