...
But how and to which extent do gasoline taxes affect the behavior of consumers? 
...
Quiz: Our data shows that the federal gas tax was increased to 18.4 cpg in 1993. How high is the federal gas tax in cents per gallon today? Just type in the amount you guess with one decimal.

Answer: 

```{r eval=FALSE}
# Run line to answer the quiz above
answer.quiz("Height of Federal Gas Tax Today")
```



Quiz: Compared to other industrial countries, do you think the U.S. has a high or low gasoline taxation?
...
```{r eval=FALSE}
# Run line to answer the quiz above
answer.quiz("Height of Federal Gas Tax Comparison")
```
...
Quiz: Before looking at the data of the absolute gasoline taxes per state - which state do you think has had the highest gasoline taxation in 2008?
...
Quiz: Check all states with a state tax higher than the federal tax in 2006.
...
```{r eval=FALSE}
# Run line to answer the quiz above
answer.quiz("State Tax Higher than Federal Tax")
```
...
Quiz: Which function do we need in order to exclude the rows with the previously stated years?
...
Quiz: What would happen if the omitted variable `crude oil price` influenced the retail oil price ($\beta_2≠0$) but is not correlated with the gasoline tax?

[1]: Beta 1 would still be biased.
[2]: Beta 1 would not be biased.

```{r eval=FALSE}
# Run line to answer the quiz above
answer.quiz("omitted variable bias")
```
...
With the newly acquired knowledge about the composition of U.S. retail gasoline prices, we may now dive deeper into the topic and examine the effects the gasoline price components have on the consumer behavior. 
...
$$q_{sy}=\beta_{0}+\beta_{1}P_{sy}+X_{sy}\theta+\delta_{s}+\phi_{s}+e_{sy}$$
...
- $X_{sy}$ is a vector of state-level covariates
...
$$(|\frac{\delta log(q)}{\delta\tau}| > |\frac{ \delta log(q)}{\delta p}|)$$.
...
Quiz: How will consumers respond, if hypothetically the retail gasoline price rises through increases of the gasoline tax and tax-exclusive gasoline price by the same amount?

[1]: Consumers react equal to both increases.
[2]: Consumers react more strongly to the increase of the tax-exclusive gasoline price.
[3]: Consumers react more strongly to the increase of the gasoline tax.

```{r eval=FALSE}
# Run line to answer the quiz above
answer.quiz("main question")
```
...
stargazer(___, ___,  
          ...
          omit = c("lntrkscap"),
          covariate.labels = c("log(gas price)", "log(1+tax ratio)", "log(tax. excl. 
          price)"))
```



Again, the coefficients of our explanatory variables ar of most interest to us. Looking at our last regression, the coefficient estimate of the tax-exclusive gasoline price `lngp` is $-0.113$ whereas the coefficient of the tax ratio `lntr` is $-0.292$. Now, both coefficient estimates are significant at the $1$% level. It is striking, that the coefficient estimate of the tax component is much larger than the one on the tax-exclusive price. Consequently, the hypothesis that the two coefficient estimates are equal can be rejected: $\beta_{1}$ and $\beta_{2}$ are not equal at the $1$% significance level. Moreover, the effect of the tax component is larger than that of the tax-inclusive retail price. The authors found this effect with a number of different specifications (cf. Li et al. (2014), p. 314).
...
Quiz: How does the tax elasticity change with a higher income?

[1]: The tax elasticity increases with income.
[2]: The tax elasticity decreases with income.

```{r eval=FALSE}
# Run line to answer the quiz above
answer.quiz("Consumer Response based on Income Quintile")
```
...
Quiz: Which of the regression coefficients is significant at the 1% level or below? Type in the result, exactly as it is named in the table above.

Answer: 

```{r eval=FALSE}
# Run line to answer the quiz above
answer.quiz("Significant Regression Coefficient")
```
...
Quiz: Please complete the following sentence. An increase of the GSP per capita by 1.000$...

[1]: decreases the state gasoline tax by 16.7%.
[2]: decreases the state gasoline tax by 0.167$.

```{r eval=FALSE}
# Run line to answer the quiz above
answer.quiz("Interpretation of Regression Coefficient")
```
...
Before starting with the analysis take a guess on the outcome: 


Quiz: We will analyze to which extent changes in the gasoline prices will affect decisions concerning vehicle purchase (represented through Miles per Gallon consumption of the vehicles) and vehicle driving (represented by Vehicle Miles Driven). Will the causal effect be stronger for the MPG or VMT as dependent variable?
...
Quiz: Take a guess. Do the characteristics described above differ much between the two samples? 
...
Quiz: Take a guess. Which household specific control variable will have the highest percentage difference between the two samples? 


[1]: the average MPG per household
[2]: the household size
[3]: the household income
[4]: the education of the reference person

```{r eval=FALSE}
# Run line to answer the quiz above
answer.quiz("Average percentage difference")
```
...
Quiz: Before performing the regression estimation, complete the following sentence. An increase of the tax component or the tax-exclusive price will have a ... effect on the MPG.
...
```{r eval=FALSE}
# Run line to answer the quiz above
answer.quiz("Causal Effect on MPG")
```
...
Quiz: Take a guess on the outcome of the regression by completing the following sentence. An increase of the tax component or the tax-exclusive price will have a ... effect on the VMT.
...
```{r eval=FALSE}
# Run line to answer the quiz above
answer.quiz("Causal Effect on VMT")
```
...
Quiz: Around which events do you observe higher amplitudes of the media coverage?

[1]: tax-exclusive price changes
[2]: tax changes

```{r eval=FALSE}
# Run line to answer the quiz above
answer.quiz("Salience of tax changes or tax-exclusive price changes")
```
...
Quiz: The coefficient of the gas tax change in the previous three months lies at $0.177$. How can this value be interpreted? Complete the following sentence. With a change of $0.01$ USD of the gas tax, the media coverage increases by ...
...
```{r eval=FALSE}
# Run line to answer the quiz above
answer.quiz("Log-level interpretation print")
```
...
Quiz: Check the box with the event that likely evokes the most media attention.

[1]: previous price change
[2]: previous tax change
[3]: upcoming price change
[4]: upcoming tax change

```{r eval=FALSE}
# Run line to answer the quiz above
answer.quiz("Tax change in previous or upcoming three months")
```
...
Quiz: If we compare the additional news coverage three months before the tax change. Would you think the additional coverage in the newspapers or in the nightly TV segments is relatively higher?
...
```{r eval=FALSE}
# Run line to answer the quiz above
answer.quiz("Ceteris Paribus effect before tax change")
```
...
Quiz: The coefficient of the gas tax change in the upcoming three months lies at $0.392$. How can this value be interpreted? Complete the following sentence. With an upcoming change of $0.01$ USD of the gas tax, the TV media coverage increases by ...
...
```{r eval=FALSE}
# Run line to answer the quiz above
answer.quiz("Log-level interpretation TV news")
```
